...
// ✅ All useState should be on top, before any useEffect
...
useEffect(() => {
  ...
  if (ed) {
    ...
    setTour_id(ed.tournament_id); // <- this should exist in editedData
  }
}, [props.editedData]);
...
useEffect(() => {
  if (!tournamentInit.current && options && props.editedData) {
    const selected = options.find(opt => opt.value === props.editedData.tournament_name);
    if (selected) {
    ...
    <>
      <div className="login" style={{ height: "auto" }}>
        <div className="form-container">
          <h2 style={{ textAlign: "center" }}>Update Match</h2>
...
/>
            ...
            >
              <CustomDateInput Setstart_date={setmatch_date} start_date={match_date} placeholder={"Match Date"} />
              <MyTimePicker match_time={match_time} onChange={setmatch_time} placeholder={"Time"} />
            </div>
...
/>
            ...
            <div className="form-group" style={{ marginTop: "1rem" }}>
              <input type="submit" name="submit" value="Update Match" style={{backgroundColor:'#0d6efd'}} />
            </div>
          </form>
        </div>
      </div>
