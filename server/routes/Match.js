...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const Is_tournament = await MatchSchema.findOne({
        match_time,
        match_date,
      });
      if (Is_tournament) {
      ...
      return res
        .status(500)
        .json({ error: "Internal Server Error or duplicate entry." });
    }
  }
);
...
router.post("/deletematch", async (req, res) => {
  ...
  try {
    ...
    const match = await MatchSchema.findOneAndDelete({
...
router.put("/update", async (req, res) => {
  ...
  try {
    ...
    const matchupdate = await MatchSchema.findByIdAndUpdate(
      fromatId,
      { $set: req.body },
      { new: true }
    );
    ...
    return res
      .status(500)
      .json({ error: error, message: "Interval Server Error" });
  }
});
